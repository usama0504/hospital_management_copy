    ...
    <div class="min-h-[85vh] flex items-center justify-center py-10 px-4 sm:px-6 lg:px-8">
        <div class="max-w-xl w-full bg-white border border-gray-100 shadow-xl shadow-gray-100/80 rounded-3xl p-8 sm:p-10">
            ...
            <div class="mb-8 text-center">
                <h2 class="text-xl sm:text-3xl font-bold text-gray-900 tracking-tight">Welcome Back</h2>
            ...
            <!-- Error Message Alert -->
            @if($errors->any())
                <div class="mb-6 flex items-center bg-rose-50 border border-rose-200 text-rose-800 p-4 rounded-2xl text-xs sm:text-sm font-semibold shadow-sm">
                    {{ $errors->first() }}
                </div>
            @endif
            ...
            <form action="{{ route('login') }}" method="POST" class="space-y-5">
                ...
                <!-- Email Field -->
                <div>
                    <label for="email" class="block text-xs font-semibold text-gray-700 uppercase tracking-wider mb-2">Email</label>
                ...
                <!-- Submit Button -->
                <div class="pt-2">
                    <button type="submit" class="w-full inline-flex items-center justify-center bg-orange-500 text-white py-3.5 rounded-xl font-bold text-sm shadow-lg shadow-orange-500/25 hover:bg-orange-600 transition">
                        Login
                    </button>
                </div>
            </form>
            ...
            <p class="mt-8 text-center text-xs sm:text-sm text-gray-500 font-medium">
                ...
                <a href="{{ route('register') }}" class="font-bold text-orange-600 hover:text-orange-700 transition ml-1 underline underline-offset-2">Register here</a>
            </p>
