    ...
    <div class="py-4">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-4 flex items-center justify-between">
                <div>
                    <h2 class="text-xl font-black text-gray-900 tracking-tight">{{ __('Edit Patient') }}</h2>
            ...
            <form action="{{ route('patients.update', $patient->id) }}" method="POST" class="bg-white border border-gray-100 shadow-sm rounded-2xl p-5 sm:p-6 space-y-4">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <textarea name="address" id="address" rows="2"
                            class="w-full bg-gray-50/50 border border-gray-200 rounded-xl px-3.5 py-2.5 text-xs font-medium text-gray-700 focus:outline-none focus:bg-white focus:border-orange-500 focus:ring-2 focus:ring-orange-500/25 transition">{{ old('address', $patient->address) }}</textarea>
                    </div>
                </div>
