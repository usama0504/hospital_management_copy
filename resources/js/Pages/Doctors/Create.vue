        ...
        <div class="py-6 sm:py-8">
            <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="mb-6 flex items-center justify-between">
                    ...
                    <Link :href="route('doctors.index')" class="text-xs font-bold text-gray-600 hover:text-gray-900 transition bg-white px-4 py-2.5 rounded-xl border border-gray-200 shadow-sm">
                        &larr; Back to Doctors
                    </Link>
                </div>

                <!-- Error Messages Alert -->
                <div v-if="Object.keys(form.errors).length > 0" class="mb-6 flex flex-col gap-1 bg-rose-50 border border-rose-200 text-rose-800 p-4 rounded-2xl text-xs font-semibold shadow-sm">
                ...
                <form @submit.prevent="submit" class="bg-white border border-gray-100 shadow-sm rounded-2xl p-6 sm:p-8 space-y-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" v-model="form.phone" id="phone" required
                                class="w-full bg-gray-50/50 border border-gray-200 rounded-xl px-4 py-3 text-xs font-medium text-gray-700 focus:outline-none focus:bg-white focus:border-orange-500 focus:ring-2 focus:ring-orange-500/25 transition" />
                            <div v-if="form.errors.phone" class="project-error text-rose-600 text-[11px] mt-1 font-semibold">{{ form.errors.phone }}</div>
                        </div>

                        <!-- Specialization Field -->
                        <div>
                            <label for="specialization" class="block text-[11px] font-bold text-gray-700 uppercase tracking-wider mb-1.5">Specialization</label>
                        ...
                        <div class="sm:col-span-2">
                            <label for="password" class="block text-[11px] font-bold text-gray-700 uppercase tracking-wider mb-1.5">Login Password</label>
                    ...
                    <div class="flex items-center gap-4 pt-4 border-t border-gray-100">
                        <button type="submit" :disabled="form.processing" class="inline-flex items-center justify-center bg-orange-500 text-white px-6 py-3 rounded-xl font-bold text-xs shadow-md shadow-orange-500/20 hover:bg-orange-600 transition disabled:opacity-50">
                            <span v-if="form.processing">Saving...</span>
                            <span v-else>Save Doctor</span>
                        </button>
                        <Link :href="route('doctors.index')" class="text-xs font-bold text-gray-600 hover:text-gray-900 transition px-4 py-2.5">
                            Cancel
                        </Link>
                    </div>
                </form>
