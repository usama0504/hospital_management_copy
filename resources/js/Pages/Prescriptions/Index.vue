                ...
                <div class="block md:hidden divide-y divide-gray-100">
                    <template v-if="prescriptions.data && prescriptions.data.length > 0">
                        <div v-for="prescription in prescriptions.data" :key="prescription.id"
                            ...
                            <div class="flex items-center justify-center gap-3.5 pt-2 border-t border-gray-50">
                                <!-- View Button -->
                                <Link :href="route('prescriptions.show', prescription.id)"
                                    class="w-7 h-7 inline-flex items-center justify-center rounded-lg bg-gray-50 text-emerald-600 hover:bg-emerald-100 transition"
                                    title="View">
                                ...
                                <Link :href="route('prescriptions.edit', prescription.id)"
                                    class="w-7 h-7 inline-flex items-center justify-center rounded-lg bg-gray-50 text-blue-600 hover:bg-emerald-100 transition"
                                    title="Edit">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                ...
                                <button v-if="isAdmin" @click="deletePrescription(prescription.id)" type="button"
                                    class="w-7 h-7 inline-flex items-center justify-center rounded-lg bg-gray-50 text-rose-600 hover:bg-emerald-100 transition"
                                    title="Delete">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <table class="min-w-full divide-y divide-gray-100 text-left text-xs font-medium text-gray-600">
                        <thead class="bg-gray-50 text-gray-400 uppercase tracking-wider font-bold text-[10px]">
                            <tr>
                                ...
                                <th class="py-3.5 px-4 lg:px-6 text-center whitespace-nowrap">Actions</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100 bg-white">
                            <template v-if="prescriptions.data && prescriptions.data.length > 0">
                                <tr v-for="prescription in prescriptions.data" :key="prescription.id"
                                    ...
                                    <td class="py-4 px-4 lg:px-6 text-center whitespace-nowrap">
