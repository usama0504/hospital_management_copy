...
// Controller se ane walay props (bill data, patients aur doctors lists)
...
// Inertia useForm helper initialized with existing bill values
const form = useForm({
    patient_id: props.bill.patient_id ?? '',
    doctor_id: props.bill.doctor_id ?? '',
    amount: props.bill.amount ?? '',
    status: props.bill.status ?? 'Unpaid',
    // Date formatting helper for input[type="date"]
    bill_date: props.bill.bill_date ? props.bill.bill_date.split('T')[0] : '',
});
    ...
    <div class="py-4">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-4 flex items-center justify-between">
                <div>
                    <h2 class="text-xl font-black text-gray-900 tracking-tight">Edit Bill</h2>
                    <p class="text-[11px] text-gray-500 font-medium">Update the billing and payment details for this record.</p>
                </div>
                <Link :href="route('bills.index')" class="text-xs font-bold text-gray-500 hover:text-gray-900 transition bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-200">
                    &larr; Back
                </Link>
            </div>
            ...
            <form @submit.prevent="submit" class="bg-white border border-gray-100 shadow-sm rounded-2xl p-5 sm:p-6 space-y-4">
                ...
                <div class="flex items-center gap-3 pt-3 border-t border-gray-100">
                    ...
                    <Link :href="route('bills.index')" class="text-xs font-bold text-gray-500 hover:text-gray-900 transition px-3 py-2">
                        Cancel
                    </Link>
                </div>
            </form>
