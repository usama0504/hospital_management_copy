        ...
        <div class="space-y-6">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-1 font-medium">Manage and monitor all hospital appointments
                        efficiently.</p>
                </div>
                ...
                <div class="flex items-center gap-2 flex-wrap">
                    ...
                    <Link :href="route('appointments.calendar')"
                        class="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 bg-white text-gray-700 border border-gray-200 px-4 py-2.5 rounded-xl font-bold text-xs hover:bg-gray-50 transition shadow-2xs">
                        <svg class="w-4 h-4 text-orange-500 shrink-0" fill="none" stroke="currentColor" stroke-width="2"
                            viewBox="0 0 24 24">
                    ...
                    <Link :href="route('appointments.create')"
                        class="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 bg-orange-500 text-white px-5 py-2.5 rounded-xl font-semibold text-xs shadow-md shadow-orange-500/20 hover:bg-orange-600 transition">
                        ...
                        <div v-for="appointment in appointments.data" :key="appointment.id"
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="w-9 h-9 rounded-full bg-orange-100 text-orange-600 font-bold flex items-center justify-center text-xs shrink-0">
                                        {{ strtoupperName(appointment.patient?.name) }}
                                    </div>
                                    <div>
                                        <h4 class="font-bold text-gray-900 text-xs">{{ appointment.patient?.name ??
                                            'N/A' }}</h4>
                                        <p class="text-[11px] text-gray-500">Dr. {{ appointment.doctor?.name ?? 'N/A' }}
                                        </p>
                                    </div>
                                </div>
                                <span :class="getStatusClass(appointment.status)"
                                    class="inline-flex items-center px-2 py-0.5 rounded-lg border text-[10px] font-bold capitalize shadow-2xs">
                                    {{ appointment.status }}
                                </span>
                            </div>

                            <div
                                class="flex items-center justify-between text-[11px] text-gray-500 pt-1 border-t border-gray-50">
                                ...
                                <div class="flex items-center gap-1.5">
                                    <Link :href="route('prescriptions.create', { appointment_id: appointment.id })"
                                        class="p-1.5 rounded-lg bg-gray-50 text-emerald-600 hover:bg-emerald-50 transition"
                                    ...
                                    <Link :href="route('appointments.edit', appointment.id)"
                                        class="p-1.5 rounded-lg bg-gray-50 text-blue-600 hover:bg-blue-50 transition"
                                        title="Edit">
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2"
                                            viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                                    ...
                                    <button @click="deleteAppointment(appointment.id)" type="button"
                                        class="p-1.5 rounded-lg bg-gray-50 text-rose-600 hover:bg-rose-50 transition"
                                        title="Delete">
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2"
                                            viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                            ...
                            <template v-if="appointments.data && appointments.data.length > 0">
                                <tr v-for="appointment in appointments.data" :key="appointment.id"
                                    ...
                                    <td class="py-4 px-6 font-bold text-gray-900">
                                        <div class="flex items-center gap-3">
                                            <div
                                                class="w-8 h-8 rounded-full bg-orange-100 text-orange-600 font-bold flex items-center justify-center text-xs">
                                                {{ strtoupperName(appointment.patient?.name) }}
                                            </div>
                                    ...
                                    <td class="py-4 px-6">
                                        <span :class="getStatusClass(appointment.status)"
                                            class="inline-flex items-center px-2.5 py-1 rounded-lg border text-[11px] font-bold capitalize shadow-2xs">
                                            {{ appointment.status }}
                                        </span>
                                    </td>
