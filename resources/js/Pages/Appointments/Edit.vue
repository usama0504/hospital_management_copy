...
const existingDateTime = formatLocalDateTime(props.appointment?.appointment_date);

const appointmentDate = ref(existingDateTime ? existingDateTime.split('T')[0] : '');
const currentTime = ref(existingDateTime ? existingDateTime.split('T')[1] : '');
...
const loadSlots = async () => {
    ...
    try {
        ...
        if (currentTime.value && !availableSlots.value.includes(currentTime.value)) {
...
const submit = () => {
    form.put(route('appointments.update', props.appointment.id), { preserveScroll: true });
};
        ...
        <div class="py-6 sm:py-8">
            <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
                ...
                <div class="flex items-center justify-between gap-4">
                    <div>
                        <h2 class="text-xl sm:text-2xl font-black text-gray-900 tracking-tight">Edit Appointment</h2>
                        <p class="text-xs text-gray-500 font-medium">Change appointment details or select a new time
                            slot.</p>
                    </div>
                ...
                <form @submit.prevent="submit"
                    class="bg-white border border-gray-100 shadow-sm rounded-2xl p-5 sm:p-6 space-y-4">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">

                        <div>
                            <label
                                class="block text-[11px] font-bold text-gray-700 uppercase tracking-wider mb-1.5">Patient</label>
                        ...
                        <div>
                            ...
                            <div v-if="form.errors.appointment_date"
                                class="text-rose-600 text-[11px] mt-1 font-semibold">{{ form.errors.appointment_date }}
                            </div>
                        </div>

                        <div>
                            <label
                                class="block text-[11px] font-bold text-gray-700 uppercase tracking-wider mb-1.5">Doctor</label>
                        ...
                        <div class="sm:col-span-2">
                            <label
                                class="block text-[11px] font-bold text-gray-700 uppercase tracking-wider mb-2">Available
                                Time Slots</label>
                    ...
                    <div
                        ...
                        <Link :href="route('appointments.index')"
                            class="w-full sm:w-auto text-center text-xs font-bold text-gray-500 px-4 py-2.5">
                            Cancel
                        </Link>
