...
// Controller se ane walay props (appointment, patients, aur doctors lists)
...
// Inertia useForm helper initialized with existing appointment data
const form = useForm({
    patient_id: props.appointment?.patient_id || '',
    doctor_id: props.appointment?.doctor_id || '',
    appointment_date: formatLocalDateTime(props.appointment?.appointment_date),
    status: props.appointment?.status || 'Scheduled',
});

// Form update handler (PUT request via Inertia)
const submit = () => {
